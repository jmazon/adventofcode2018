...
type Pos = (Int,Int)
data Cell = Empty | Wall | Unit { uId :: Int, uType :: Bool, uHp :: Int }
          deriving Eq
...
turn ep g0 = step g0 units where
  ...
  step g ((p,i):us) | not (isUnit u) || uId u /= i = step g us
                    | null targets = -- no targets -> end of combat
                        Left (sum [ uHp u | u <- elems g, isUnit u ])
                     -- no reachable range -> end of unit turn
                    | null reachable = step g us
                    | [] <- closest = attack g p u us -- attack
                     -- move
                    | otherwise = attack (g // [(p,Empty),(p',u)]) p' u us
    where
      u = g!p
      targets = filter (((&&) <$> isUnit <*> enemies u) . snd) (assocs g)
      tRange = filter ((||) <$> (== p) <*> isFree . (g!)) $
               concatMap (neighbors . fst) targets
      reachable = bfs g tRange S.empty [[p]]
      (closest:_) = sortBy (comparing length <> comparing head <> comparing last)
                           reachable
      p' = last closest

  attack g p u us
    | null targets = step g us -- no targets, end of unit turn
    | newHp <= 0 = if uIsElf (g!target) && ep > 3
                   then Left (-1)
                   else step (g // [(target,Empty)]) us
    | otherwise = step (g // [(target,(g!target) { uHp = newHp })]) us
    where targets = filter (((&&) <$> isUnit <*> enemies u) . (g!)) (neighbors p)
  ...
  bfs g gl cl ts@ ~(t@(p:_):ts')
    ...
    | otherwise = bfs g gl cl' (ts' ++ ts'')
      where
        cl' = S.insert p cl
        ts'' = map (:t) $ filter (`S.notMember` cl) $
               filter (isFree . (g!)) $ neighbors p
...
main = do
  ...
  let best = bSearch isElfWin 3 high
